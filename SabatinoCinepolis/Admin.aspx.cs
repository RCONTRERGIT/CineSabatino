...
public partial class Admin : System.Web.UI.Page
{
    ...
    protected void gvPeliculas_Sorting(object sender, GridViewSortEventArgs e)
    {
        try
        {
            ...
            LlenarOrdenado(null, columna, direccion);
        }
        catch (Exception ex)
        {
            Title = ex.Message;
    ...
    private void LlenarOrdenado(int? fila, string columna, string direccion)
    {
        List<EntPelicula> lst5 = new BusPelicula().Obtener();
        switch (columna)
        {
            case "nombre":
                {
                    if (direccion == "ASC")
                        lst5 = lst5.OrderBy(x => x.nombre).ToList();
                    else
                        lst5 = lst5.OrderByDescending(x => x.nombre).ToList();
                    break;
                }
            default:
                {
                    Title = "Upss opción no válida";
                    break;
                }
        }
        gvPeliculas.DataSource = lst5;
        gvPeliculas.DataBind();
